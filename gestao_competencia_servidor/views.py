...
from .models import ConsultaCompetencia, Competencia, Supervisor
...
def novas_formacoes_previstas(request):
    return render(request, 'novas_formacoes_previstas.html')

def novas_formacoes_realizadas(request):
    return render(request, 'novas_formacoes_realizadas.html')
...
def adicionar_membro(request):
    if request.method == 'POST':
        ...
        supervisor_nome = request.POST['supervisor']  # Obtenha o valor do campo de supervisor
        area = request.POST['area']  # Obtenha o valor do campo de área

        # Verifica se todos os campos obrigatórios estão presentes
        if not id or not nome or not email or not cargo_id or not setor_id or not supervisor_nome:
            mensagem_erro = 'Por favor, preencha todos os campos obrigatórios.'
            setores = Setor.objects.all()
            cargos = Cargo.objects.all()
            supervisores = Supervisor.objects.all()
            return render(request, 'adicionar_membro.html',
                          {'setores': setores, 'cargos': cargos, 'supervisores': supervisores, 'mensagem_erro': mensagem_erro})

        # Recupera o setor e cargo com base nos IDs fornecidos
        setor = Setor.objects.get(id=setor_id)
        cargo = Cargo.objects.get(id=cargo_id)
        supervisor = Supervisor.objects.get(nome=supervisor_nome)

        # Cria o membro
        membro = Membro(id=id, nome=nome, setor=setor, cargo=cargo, email=email, supervisor=supervisor_nome, area=area)
        membro.save()
    ...
    return render(request, 'adicionar_membro.html', {'setores': setores, 'cargos': cargos, 'supervisores': supervisores})
...
def formacoes_previstas(request):
    ...
    ano_filtrado = request.GET.get('ano')

    # Consultar os anos únicos presentes na coluna datainicial da tabela FormacaoPrevista
    anos_disponiveis_qs = FormacaoPrevista.objects.annotate(
        datainicial_date=Cast('datainicial', output_field=DateField())
    ).annotate(
        ano=ExtractYear('datainicial_date')
    ).values_list('ano', flat=True).distinct()

    # Remover a opção "None" da lista de anos disponíveis
    anos_disponiveis = [ano for ano in anos_disponiveis_qs if ano is not None]

    # Verificar se ano_filtrado não é None e é uma string válida antes de converter
    if ano_filtrado is not None and ano_filtrado.isdigit():
        ano_filtrado = int(ano_filtrado)
        # Converta a datainicial para um tipo de dados DATE antes de extrair o ano
        formacoes = formacoes.annotate(
            datainicial_date=Cast('datainicial', output_field=DateField())
        ).annotate(
            ano=ExtractYear('datainicial_date')
        ).filter(ano=ano_filtrado)

        # Ordenar as formações pela data inicial, da mais antiga para a mais recente
        formacoes = formacoes.order_by('datainicial')
    ...
    for formacao in formacoes:
        ...
        # Verificar se formacao.datainicial é uma string e convertê-la para um objeto DateField, se necessário
        if isinstance(formacao.datainicial, str):
            formacao.datainicial = parse_date(formacao.datainicial)

        # Verificar se formacao.datafinal é uma string e convertê-la para um objeto DateField, se necessário
        if isinstance(formacao.datafinal, str):
            formacao.datafinal = parse_date(formacao.datafinal)


        # Formatando as datas para o formato desejado
        if formacao.datainicial:
            formacao.datainicial = DateFormat(formacao.datainicial).format('d/m/Y')
        if formacao.datafinal:
            formacao.datafinal = DateFormat(formacao.datafinal).format('d/m/Y')

    context = {
        'formacoes_previstas': formacoes,
        'formacao_prevista': None,  # Pode ser None, já que estamos na página de listagem.
        'anos_disponiveis': anos_disponiveis,  # Inclua esta lista no contexto
    }
...
def adicionar_editar_formacao_prevista(request, formacao_id=None):
    ...
    if request.method == 'POST':
        ...
        formacao_prevista.status = status
